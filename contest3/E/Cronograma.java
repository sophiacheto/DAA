...
class Graph {
    ...
    public void find(int[] perguntas) {
        ...
        for (int perg : perguntas) {
            int curr = 0;
            for (int instante : eventos.keySet()) {
                if (instante > perg) 
                    break;
                curr += eventos.get(instante);
            }
            System.out.println(perg + " " + curr);
        }}
}
...
public class Cronograma {
    public static void main(String[] args) {
        ...
        int qtd_perguntas = scan.nextInt();
        int[] perguntas = new int[qtd_perguntas];

        for (int j=0; j<qtd_perguntas; j++) 
            perguntas[j] = scan.nextInt();

        tarefas.find(perguntas);
        // tarefas.picos();  
    }
}
